        ...
        <div className="w-full max-w-[420px] max-h-[90vh] bg-[#272320] border border-[#433D37] rounded-3xl overflow-hidden text-[#F1E2CB] shadow-2xl flex flex-col my-auto">
          ...
          <div className="flex items-center justify-between p-3.5 px-4 border-b border-[#3A332C] bg-[#191715]/60">
            <span className="text-[11px] font-sans font-medium text-[#D9B98D] uppercase tracking-wider flex items-center gap-1.5">
              ...
              {isDeviantArt ? 'DeviantArt' : t('navigation.references')}
            </span>
            <button
              type="button"
              onClick={onClose}
              aria-label={t('common.close')}
              className="p-1 rounded-full text-[#A99D8E] hover:text-[#FDF8F0] hover:bg-[#332E2A] transition-colors cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="relative w-full aspect-square bg-[#191715] overflow-hidden group">
            ...
            <button
              ...
              aria-label={t('artwork.expand')}
              className="absolute bottom-3 right-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#191715]/90 border border-white/20 text-[#FDF8F0] hover:bg-[#191715] text-xs font-sans font-medium transition-transform active:scale-95 shadow-lg backdrop-blur-sm cursor-pointer"
            >
              <Maximize2 className="w-3.5 h-3.5 text-[#D9B98D]" />
              <span>{t('artwork.expand')}</span>
            </button>
          </div>
          ...
          <div className="p-4 sm:p-5 space-y-3.5 overflow-y-auto no-scrollbar max-h-[40vh] text-left">
            <div>
              <h3 className="font-display text-[20px] font-semibold text-[#FDF8F0] leading-tight">
                {artwork.title}
              </h3>
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              ...
              {artwork.tags?.slice(0, 5).map((tVal, idx) => (
                <span
                  key={idx}
            ...
            <div className="pt-2 flex items-center gap-2 border-t border-[#3A332C]">
              {artwork.sourceUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 py-2.5 rounded-full border border-[#433D37] text-xs font-sans text-[#A99D8E] hover:text-[#FDF8F0] hover:bg-[#332E2A] flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                >
                  <span>{t('artwork.viewOriginal')}</span>
              ...
              <button
                ...
                className={`flex-1 py-2.5 rounded-full text-xs font-sans font-medium flex items-center justify-center gap-1.5 transition-all shadow-sm cursor-pointer ${
                  isSaved
              ...
              >
                {isSaved ? (
                  <>
                    ...
                    <span>{t('artwork.savedToReference')}</span>
                  </>
                ) : (
                  <>
                    <Bookmark className="w-4 h-4" />
                    <span>{isSaving ? t('upload.uploading') : t('artwork.saveReference')}</span>
                  </>
                )}
              </button>
            </div>
