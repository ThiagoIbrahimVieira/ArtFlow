...
interface TokenInfo {
  ...
  expiryTimestamp: number; // epoch ms when token expires
}
 ...
 */
export async function fetchDailyDeviations(params: DeviantArtQuery): Promise<any[]> {
  ...
  const normalized = json.results
    .filter((item: any) => {
      ...
      if (!item.content || !item.content.src) return false;
      if (!item.title || !item.author || !item.author.userid) return false;
      return true;
    })
    .map((item: any) => ({
      id: item.deviationid,
